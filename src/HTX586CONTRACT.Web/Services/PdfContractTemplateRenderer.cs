    ...
    ILogger<PdfContractTemplateRenderer> logger)
{
    ...
    private static void NormalizeDocumentMetadata(PdfSharp.Pdf.PdfDocument document, Contract contract)
    {
        ...
        var businessTitle = ContractBusinessTitle(contract.BusinessType);
        info.Elements.SetString("/Title", $"{businessTitle} {First(contract.ContractNumber, contract.Id.ToString("N"))}");
        info.Elements.SetString("/Author", "HTX586CONTRACT");
        info.Elements.SetString("/Subject", businessTitle);
        info.Elements.SetString("/Keywords", $"hợp đồng, {businessTitle.ToLowerInvariant()}, HTX 586");
    ...
    private Dictionary<string, string> BuildTextValues(Contract contract)
    {
        ...
        var driverName = snapshot is not null
            ...
            : First(contract.OperatingDriverName, contract.DriverNameSnapshot, driver?.FullName, "...");
        ...
        var values = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["CONTRACT_TITLE"] = businessTitle,
            ["CONTRACT_BUSINESS_TYPE"] = businessTitle,
    ...
    private Dictionary<string, string?> BuildImageValues(Contract contract)
    {
        ...
        return new Dictionary<string, string?>(StringComparer.Ordinal)
        {
            ...
            // Người lái thực tế có thể khác Chủ xe. Chỉ dùng chữ ký tài xế nếu
            // từng có chữ ký riêng theo hợp đồng; tuyệt đối không lấy chân ký Chủ xe thay thế.
            ["SIG_DRIVER"] = ContractSignaturePath(contract, SignatureParty.Driver),
