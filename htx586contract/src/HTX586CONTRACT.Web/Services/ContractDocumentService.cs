    ...
    ILogger<ContractDocumentService> logger) : IContractDocumentService
{
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                if (IsFinalized(contract))
                    throw new InvalidOperationException("Hợp đồng đã hoàn thành, có PDF chính thức và bị khóa.");
    ...
    public async Task<string> GeneratePdfAsync(Guid contractId, CancellationToken ct = default)
    {
        ...
        if (contract.Status is ContractStatus.Cancelled or ContractStatus.Expired or ContractStatus.Invalidated)
            throw new InvalidOperationException("Hợp đồng đã hủy, hết hạn hoặc vô hiệu hóa nên không thể tạo PDF.");

        // PDF phải được tạo thành công trước khi đổi trạng thái Completed. Nhờ vậy,
        // nếu render PDF lỗi thì hợp đồng vẫn ở trạng thái chờ ký và vẫn chỉnh sửa được.
    ...
    private static bool IsFinalized(Contract contract)
        => contract.Status == ContractStatus.Completed &&
           !string.IsNullOrWhiteSpace(contract.PdfFileUrl) &&
           !string.IsNullOrWhiteSpace(contract.PdfSha256) &&
           contract.PdfGeneratedAt.HasValue;
